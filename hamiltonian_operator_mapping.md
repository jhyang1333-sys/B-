...
> **版本更新 (2025)**：代码实现已从不稳定的“强形式”（直接计算二阶导）迁移至**“弱形式”（分部积分）**，并修正了原论文在质量极化项上的系数笔误。
...
1. **算符分组**：先按类型拆分（纯径向、混合导数、角向耦合、势能）。
2. **弱形式积分**：动能项使用 \(\langle \phi' | \psi' \rangle\) 形式，避免 \(r=0\) 处的数值奇异性。
3. **幂次**：遇到 \(r_{12}\) 相关几何因子时，**先**与基函数幂次合并，得到标准形 \(r_{12}^{c_{ij}^{(n)}}\)。
4. **数值**：对几何因子 \((r_1^2 - r_2^2)/r_1\) 采用因式分解计算，防止大数相减误差。
...
| **动能项 (弱形式)**      | \(-\frac{1}{2\mu}\nabla^2\)                                        | `I` (一阶导数积) |
| 纯 \(r_{12}\) 导数       | \(\partial_{r_{12}},\ \partial_{r_{12}}^2\)                        | `I`              |
| **混合导数**（核心难点） | \(\partial_{r_1}\partial_{r_{12}},\ \partial_{r_1}\partial_{r_2}\) | `I` (需拆分幂次) |
| 角向耦合                 | \(\hat r\cdot\hat\nabla^Y\), \(\hat\nabla^Y\cdot\hat\nabla^Y\)     | `J`              |
| 势能项                   | \( -2/r_1 - 2/r_2 + 1/r_{12} \)                                    | `I`              |

## 4. 动能项 (Kinetic Energy) - 弱形式

原论文公式 (2.8) 为 \(-\frac{1}{2\mu} \nabla^2\)。为保证数值稳定性，我们在代码中采用分部积分后的**弱形式**：

\[
\langle \Phi | -\frac{1}{2\mu} \nabla^2 | \Psi \rangle = \frac{1}{2\mu} \int d\tau \nabla \Phi \cdot \nabla \Psi
\]

| 分量       | 积分形式                                               | 对应代码系数                 |
| :--------- | :----------------------------------------------------- | :--------------------------- |
| 径向部分   | \(\frac{1}{2\mu} \int (B_i' B_k') B_j B_l + \dots\)    | `pref_kinetic_weak = 0.5/mu` |
| 离心势部分 | \(\frac{l(l+1)}{2\mu} \int \frac{B_i B_k}{r^2} \dots\) | `pref_centrifugal`           |

> **注意**：此处无需再计算 B 样条的二阶导数，极大地提高了在 \(r \to 0\) 处的精度。
...
**重要修正**：论文公式 (2.37) 中质量极化项系数写为 \(-\frac{1}{2M} (\dots \frac{1}{2r_1 r_2})\)，即 \(-1/(4M)\)。这是**笔误**。
标准质量极化算符 \(H_{MP} = -\frac{1}{M} \nabla_1 \cdot \nabla_2\) 展开后的正确径向系数应为 **\(-1/(2M)\)**。代码已修正为 `pref_cross = -0.5/M`。
...
**数值稳定性优化**：
在代码中，几何因子 \(\frac{r_1^2-r_2^2}{r_1}\) 被重写为 `(r1 - r2) * (r1 + r2) / r1`，并添加了 `_EPS_DENOM` 保护，以防止 \(r_1 \approx r_2\) 时的数值抵消。

积分拆分：
- `I(c_{ij}^{(-2)})`：径向系数 \(C_{M\mu}\, c_j \frac{r_1^2-r_2^2}{r_1}\, \frac{B_k' B_l}{B_k B_l}\)
- `I(c_{ij}^{(0)})`：径向系数 \(C_{M\mu}\, c_j \frac{1}{r_1}\, \frac{B_k' B_l}{B_k B_l}\)

### 6.2 \(\partial_{r_1}\partial_{r_2}\) 项 (质量极化径向部分)

算符（修正后）：
\[
H_{n} = -\frac{1}{2M}\, \frac{r_1^2 + r_2^2 - r_{12}^2}{r_1 r_2}\, \partial_{r_1}\partial_{r_2}
\]

积分拆分：
- `I(c_{ij}^{(0)})`：系数 \(-\frac{1}{2M} \frac{r_1^2 + r_2^2}{r_1 r_2}\) (代码使用 `ratio_cross`)
- `I(c_{ij}^{(2)})`：系数 \(+\frac{1}{2M} \frac{1}{r_1 r_2}\) (来源于 \(-(-r_{12}^2)\))

## 7. 角向耦合 (`J` 积分)

按论文中三类耦合拆分，需同步管理径向前因子与角向张量。
...
- `J(c_{ij}^{(0)})`：径向系数 \(-\tfrac{1}{M r_2}\, \tfrac{B_k' B_l}{B_k B_l}\)
- `J(c_{ij}^{(-2)})`：径向系数 \(\left(\tfrac{1}{\mu}-\tfrac{1}{M}\right) c_j\, \tfrac{r_1}{r_2}\)
...
- 仅贡献 `J(c_{ij}^{(0)})`，径向系数 \(-\tfrac{1}{M r_1 r_2}\)

## 8. 势能项

| 项                                   | 映射到的积分                                                   |
| ------------------------------------ | -------------------------------------------------------------- |
| \(-\tfrac{2}{r_1} - \tfrac{2}{r_2}\) | `I(c_{ij}^{(0)})`                                              |
| \(+\tfrac{1}{r_{12}}\)               | `I(c_{ij}^{(-1)})`（依赖 `CorrelationExpansion` 的勒让德展开） |

## 9. 实现检查清单

- [x] **弱形式动能**：已移除 `pref_second` 项，改为计算 `d1_row * d1_col`。
- [x] **系数修正**：质量极化项已使用 `-0.5/M`。
- [x] **数值防御**：混合导数几何因子已应用因式分解。
- [x] **体积元**：积分核内已包含 `measure = r1^2 * r2^2`（配合纯 1D 权重）。
